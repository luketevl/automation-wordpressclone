...
do
        ...
        mysql -u ${DBUSER} -h ${DBHOST} --password=${DBPASS} -e "create database ${DBNAME};"
        mysqldump -u "${DBUSER}" -p${DBPASS} -h ${DBHOST} --compress --single-transaction wp-lpmodelo | mysql --host=${DBHOST} -f --user=${DBUSER} --password=${DBPASS} ${DBNAME}
         wp db query --allow-root --path=../../${client}   'update wp_options set option_value = "'${URL}'" where option_name = "siteurl" or option_name = "home"' --skip-column-names
	fi
done
